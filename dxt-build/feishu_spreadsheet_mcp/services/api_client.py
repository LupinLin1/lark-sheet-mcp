...
class FeishuAPIClient:
    ...
    async def read_range(
        ...
        Returns:
            API response data
        """
        endpoint = f"sheets/v2/spreadsheets/{spreadsheet_token}/values/{range_spec}"
        params = {}
    ...
    async def find_cells(
        ...
        # Required parameters
        if "range_spec" in kwargs:
            find_condition["range"] = kwargs["range_spec"]
        else:
            raise ValueError("range_spec is required for find_cells")

        # Optional parameters with defaults
        find_condition["match_case"] = kwargs.get("match_case", False)
        find_condition["match_entire_cell"] = kwargs.get("match_entire_cell", False)
        find_condition["search_by_regex"] = kwargs.get("search_by_regex", False)
        find_condition["include_formulas"] = kwargs.get("include_formulas", False)

        data = {"find_condition": find_condition, "find": kwargs.get("find_text", "")}
